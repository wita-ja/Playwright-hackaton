export function Game(aiPlayer, difficulty) {
...
export function SmallBoard(location) {
...
export function Move(smallBoardLocation, cellLocation) {
...
export function BigBoardDisplay() {
  this.element = $("#game");
    ...
    for (var j = 0; j < 3; j++) {
      ...
      smallBoardDisplay.update(
        possibleMoves.filter(function (move) {
          return (
            move.smallBoardLocation.row === smallBoardDisplay.location.row &&
...
export function SmallBoardDisplay(parent, location) {
  this.element = $(
...
export function CellDisplay(parent, location) {
...
CellDisplay.prototype.update = function (move) {
  if (!move) {
    this.element.removeClass("enabled").addClass("disabled");
    var winner =
      game.bigBoard.children[this.parent.location.row][this.parent.location.col]
    ...
    if (room.game.players.length < 2) {
      ...
      connectionStatus.text(
        room.game.players.length === 1
          ? "One of the players has left"
          : "Both of the players have left"
      );
    }
  });
